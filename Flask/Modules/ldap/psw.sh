...
pswadmin=$(/usr/sbin/slappasswd -h {SSHA} -s "$#userrootadmin-pwd#$")
...
ldapmodify -H ldap://localhost -x -D "cn=admin,dc=ldap,dc=organization,dc=com" -w $#ldap-admin-pwd#$ -f /ldif_files/admin_update_fixed.ldif
rm /ldif_files/admin_update_fixed.ldif