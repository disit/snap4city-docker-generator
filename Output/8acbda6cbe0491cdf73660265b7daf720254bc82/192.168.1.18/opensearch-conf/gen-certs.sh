...
docker run --rm -v $PWD:/conf/ disitlab/personaldata:v3.1 keytool -import -file /conf/root-ca.pem -alias snap4ca -storepass Wx703cC1Vyb8ipAr -storetype pkcs12 -noprompt -keystore /conf/trust-store.p12
...
docker run --rm -v $PWD:/conf/ disitlab/personaldata:v3.1 keytool -importcert -keystore /conf/cacerts -file /conf/root-ca.pem -alias Wx703cC1Vyb8ipAr -storepass changeit -noprompt
