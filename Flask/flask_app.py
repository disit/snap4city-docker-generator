...
def create_app():
    ...
    def make_more_opensearch():
        if request.method == 'GET':  # if you didn't use a form you shouldn't be here
            ...
            snap4.make_multiple_opensearch_added(int(fine_as_is["$#how-many-opensearch#$"]),fine_as_is['dashboard'],fine_as_is['LDAP'], open_ips_keys, alt_out='./Output/opensearch-temp')
    ...
    def some_kind_of_error(e):
        return render_template('500.html', reason=e,helpmail=os.environ["help_mail"],version=os.environ["version"]), 500
    ...
    def dealing_with_placeholders():
        if request.method == 'GET':  # if you didn't use a form you shouldn't be here
            ...
            if not re.match('^[abcdef\d]{40}$', token):
                ...
                return render_template('error.html',helpmail=os.environ["help_mail"],version=os.environ["version"], reason="An illegal token was used", code=400), 400
